...
class TrialClassificationModel(BaseModel):
    ...
    root_cause: str = Field(description="1-2 sentence explanation of what caused this outcome")
...
class TrialClassification:
    ...
    @classmethod
    def from_model(
        cls, trial_name: str, model: TrialClassificationModel, reward: float | None = None
    ) -> TrialClassification:
...
class BaselineValidation:
    ...
    def issues(self) -> list[str]:
        ...
        if self.nop and not self.nop.is_expected:
            issues.append("CRITICAL: nop agent passed - task may be pre-solved or tests are broken")
        if self.oracle and not self.oracle.is_expected:
            issues.append("CRITICAL: oracle agent failed - reference solution doesn't work")
        return issues
